            ...
            {items.map((item, index) => (
              <div className={`carousel-item ${index === 0 ? 'active' : ''}`} key={index}>
                ...
                <div className="carousel-caption d-flex flex-column align-items-center justify-content-center">
                  <div className="p-3">
                    <h4 className="text-white text-uppercase mb-md-3">{item.title}</h4>
                    <h1 className="display-1 text-white mb-md-4">{item.subtitle}</h1>
                    <a href={item.buttonLink} className="btn btn-primary py-md-3 px-md-5 mt-2">
                      {item.buttonText}
                    </a>
                  </div>
                </div>
...
function Index() {
  ...
  return (
    ...
    <button onClick={handleScrollToTop} className="btn btn-lg btn-primary btn-lg-square back-to-top"><i className="fa fa-angle-double-up" /></button>
  </div>
  );
};
